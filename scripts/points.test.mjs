...
test('partial run preserves missing scores and computes proportional observed points',()=>{const r=JSON.parse(readFileSync(new URL('../data/points-runs.json',import.meta.url))).runs.find(r=>r.id==='2026-09-03_2026-09-09-v0.3');const s=calculate(r);assert.equal(s.quality,null);assert.equal(s.complete,false);assert.equal(s.analytics[0].points,null);assert.ok(Math.abs(s.total-43.93911097222223)<1e-8);});
